...
const handleEvaluateSuccess = (result) => (dispatch, getState) => {
  ...
  dispatch(resourcesActions.addFile('output.result', resource.name, resource.contentType, resource.content));
...
const handleEvaluateProgress = (status, info) => (dispatch, getState) => {
  if (status === 'callStateInfo:add') {
    ...
    dispatch(resourcesActions.findAndAddResources(
...
const handleLoadMoreSuccess = (result, path, transformedPath) => (dispatch, getState) => {
  ...
  const valueParent = valuePath.slice(0, -1).reduce((obj, part) => obj[part], newInfo);
  valueParent[valuePath[valuePath.length - 1]] = value;
  dispatch(timelineActions.updateInfo(newInfo));

  dispatch(resourcesActions.findAndAddResources(
    transformData(newInfo[valuePath[0]])
  ));
...
export const actionCreators = {
  ...
  evaluateService: ({ servicePath, bodyProps, bodyOptions }) => ({
    ...
    promise: ({ dispatch }) =>
      server.evaluateService(servicePath, bodyProps, bodyOptions, (message) => {
        dispatch(handleEvaluateProgress(
          message.status,
  ...
  loadMore: ({ path, transformedPath }) => ({
    ...
    promise: ({ dispatch }) => {
      ...
      return server.loadMore(path, transformedPath)
        .then(result => dispatch(handleLoadMoreSuccess(result, path, transformedPath)))
    },
    path,
    transformedPath,
  }),
